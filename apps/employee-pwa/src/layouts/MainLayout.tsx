...
const UnreadBadge: React.FC = () => {
    ...
    return (
        <motion.span
            ...
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute -top-1.5 -right-1.5 h-4 w-4 bg-red-500 text-white text-[9px] flex items-center justify-center rounded-full border-2 border-white"
        >
            {count > 9 ? '+9' : count}
        </motion.span>
    )
                ...
                <nav className="flex-1 space-y-2">
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            to={item.path}
                            className={({ isActive }) => `
                                flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-300 font-bold relative
                                ${isActive
                ...
                <div className="max-w-md mx-auto h-20 glass-card rounded-[2rem] flex items-center justify-around px-4">
                    {navItems.map((item) => (
                        <NavLink
                            ...
                            to={item.path}
                            className={({ isActive }) => `
                                flex flex-col items-center gap-1.5 px-4 py-2 rounded-2xl transition-all duration-300 relative
